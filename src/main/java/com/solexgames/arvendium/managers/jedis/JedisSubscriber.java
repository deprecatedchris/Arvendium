...
import com.solexgames.arvendium.utils.PlayerUtils;
...
public class JedisSubscriber {
    ...
    private Jedis jedis;
    private Arvendium main;

    public JedisSubscriber(Arvendium main) {
        // TODO: Make this read from the configuration.
        this.main = main;
                ...
                if (channel.equalsIgnoreCase("permissions")) {
                    ...
                    if (action == JedisSubscriberAction.ADD_RANK_PERMISSION) {
                        ...
                        if (rank != null) {
                            ...
                            if (player2 != null) player2.sendMessage(ChatColor.GREEN + "Permission '" + permission2 + "' successfully added to rank named '" + rank.getData().getName() + "'.");
                            for (Profile profile2 : Profile.getProfiles()) if (profile2.getActiveGrant().getRank().getUuid().equals(rank.getUuid())) profile2.setupAttachment();
                        }
                        return;
                    }
                    if (action == JedisSubscriberAction.DELETE_RANK_PERMISSION) {
                        ...
                        if (rank != null) {
                            ...
                            Player player2 = Bukkit.getPlayer(payload.get("player").getAsString());
                            if (player2 != null) player2.sendMessage(ChatColor.GREEN + "Permission '" + permission2 + "' successfully removed from rank named '" + rank.getData().getName() + "'.");
                            for (Profile profile2 : Profile.getProfiles()) if (profile2.getActiveGrant().getRank().getUuid().equals(rank.getUuid())) profile2.setupAttachment();
                        } return;
                    }
